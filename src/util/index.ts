  ...
  try {
    ...
    const error_ =
      error instanceof ExitPromptError
        ? new TypeError(chalk.red('Workspace selection prompt exited.'))
        : error
    throw error_
  }
}

export async function promptProjectLinkSelection(
  projects: App[],
): Promise<App> {
  ...
  try {
    ...
    const error_ =
      error instanceof ExitPromptError
        ? new TypeError(chalk.red('Project selection prompt exited.'))
        : error
    throw error_
  }
}
...
export async function confirmOverwriteCiHypFile(): Promise<boolean> {
  return inquirer.confirm({
    default: true,
    message:
      'A ci-modus-build.yml file already exists. Would you like to overwrite it?',
  })
}
...
export async function confirmExistingProjectLink(): Promise<boolean> {
  return inquirer.confirm({
    default: true,
    message:
      'You have existing projects with no linked repositories. Would you like to select from these projects?',
  })
}
...
export async function readSettingsJson(filePath: string): Promise<{
  content: string
  email: null | string
...
export async function writeToSettingsFile(
  ...
  const newSettingsContent: {
    HYP_EMAIL: string
    HYP_API_KEY: string
    HYP_WORKSPACE_ID: string
    INSTALLATION_IDS: { [key: string]: string } | null
  } = {
    HYP_EMAIL: email,
    HYP_API_KEY: apiKey,
...
export async function writeGithubInstallationIdToSettingsFile(
  gitOwner: string,
