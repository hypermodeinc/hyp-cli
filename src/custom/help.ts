...
export default class CustomHelp extends Help {
  ...
  formatTopics(topics: Interfaces.Topic[]): string {
    ...
    for (const topic of topics) {
      ...
      out +=
        ...
        ' '.repeat(Math.max(1, this.pre_pad + this.post_pad - topic.name.length)) +
        topic.description +
        '\n'
    }
    return out.trim()
  ...
  formatRootFooter(): string {
    ...
    out +=
      'View the docs:' +
      ' '.repeat(Math.max(1, this.pre_pad + this.post_pad - 12)) +
  ...
  async showRootHelp(): Promise<void> {
    ...
    this.post_pad =
      6 + this.pre_pad + this.post_pad > this.target_pad
        ? 6 + this.pre_pad + this.post_pad - this.target_pad
        : this.target_pad - this.pre_pad
    this.pre_pad += 2
  ...
  async showCommandHelp(command: Command.Loadable): Promise<void> {
    ...
    if (flags.length) {
      ...
      for (const flag of Object.values(command.flags))
        this.log(
          '  ' + chalk.bold.blueBright('--' + flag.name) + ' '.repeat(margin - flag.name.length) + flag.description,
        )
    }
