...
export default class CustomHelp extends Help {
  ...
  formatCommand(command: Command.Loadable): string {
    let out = "";
    out += chalk.bold("Usage:") + " " + chalk.bold.blueBright("hyp") + " " + command.id;
    return out;
  }
  ...
  formatRoot(): string {
    let out = "";
    out += chalk.bold.blueBright("Hypermode") + " - Build Intelligent APIs. \n\n";
  ...
  formatRootFooter(): string {
    let out = "";
    out += "View the docs:" + " ".repeat(Math.max(1, this.prePad + this.postPad - 12)) + chalk.blueBright("https://docs.hypermode.com") + "\n";
  ...
  async showCommandHelp(command: Command.Loadable): Promise<void> {
    ...
    this.log(chalk.bold.blueBright("Hypermode") + " Help " + chalk.dim("(v0.0.0)") + "\n");

    if (command.description) this.log(chalk.dim(command.description));

    this.log(chalk.bold("Usage:") + " " + chalk.bold("hyp " + name) + (args.length > 0 ? " [...args]" : "") + (flags.length > 0 ? chalk.blueBright(" [...flags]") : "") + "\n");
    // if (examples) {
    //     this.log();
  ...
  async showRootHelp(): Promise<void> {
    ...
    this.postPad = 6 + this.prePad + this.postPad > this.targetPad ? 6 + this.prePad + this.postPad - this.targetPad : this.targetPad - this.prePad;
    this.prePad += 2;
